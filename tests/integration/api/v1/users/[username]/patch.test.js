...
describe("PATCH /api/v1/users/[username]", () => {
  ...
  describe("Default user", () => {
    ...
    test("With duplicated 'username'", async () => {
      ...
      await orchestrator.createUser(expectedUsers[0]);
      const createdUser2 = await orchestrator.createUser(expectedUsers[1]);

      const activatedUser2 = await orchestrator.activateUser(createdUser2);
      const userSessionObject2 = await orchestrator.createSession(
        activatedUser2.id,
      );

      const response = await fetch(
        `http://localhost:3000/api/v1/users/${expectedUsers[1].username}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Cookie: `session_id=${userSessionObject2.token}`,
          },
          body: JSON.stringify({
    ...
    test("With `update:user:others` targeting `defaultUser`", async () => {
      ...
      const response = await fetch(
        `http://localhost:3000/api/v1/users/${defaultUser.username}`,
        {
          ...
          body: JSON.stringify({
            username: "Alterado por privilegiado",
          }),
        },
      );
